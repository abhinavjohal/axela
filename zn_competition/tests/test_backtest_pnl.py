...
class TestBacktestPnL(unittest.TestCase):
    def test_fee_schedule_per_leg(self) -> None:
        result = run_backtest(generate_synthetic_quotes(50), week=1)
        self.assertEqual(
    ...
    def test_line_by_line_net_sums_to_realized(self) -> None:
        result = run_backtest(generate_synthetic_quotes(30), week=1)
        if result.pnl_lines:
    ...
    def test_net_equals_realized_plus_mark(self) -> None:
        result = run_backtest(generate_synthetic_quotes(40), week=1)
