...
from zn_competition.risk import ExecutionException, enforce_order_size, validate_order
...
class TestExecutionCap(unittest.TestCase):
    def test_enforce_raises_on_breach(self) -> None:
        with self.assertRaises(ExecutionException) as ctx:
            enforce_order_size(5, position=8)
